//! The Kitty graphics protocol, generated by the viewer and only by the viewer.
//!
...
//! viewer:
//!
//! * `q=2` on every render command. The terminal answers graphics commands with
//!   APC sequences *on stdin*, which would land among the keystrokes being
//!   translated into page input: noise at best, a terminal's reply forwarded
//!   into the page as typing at worst. The cost is silent render errors.
//! * Direct transmission (`t=d`) only. File paths, temporary files and shared
//!   memory are faster and work only when the terminal is on this machine, and
//!   working over SSH is half the point.
...
//!   blank pane and no diagnosis.
//! * Explicit deletion of the previous frame, after the new one is placed, or a
//!   terminal asked to hold thousands of images will hold them.
...
/// that also has to hold a JPEG decode and a box filter.
...
///
/// The trick in the second half is the part that makes it work. A terminal that
/// does not know the graphics protocol does not answer the query: it says
/// nothing, and there is no timeout that distinguishes "not supported" from
/// "slow". So the probe is followed by a Primary Device Attributes request,
/// which every terminal since the VT100 answers. Read until one of them arrives:
/// a graphics reply means yes, a device-attributes reply arriving *first* means
/// the graphics query was silently dropped.
///
