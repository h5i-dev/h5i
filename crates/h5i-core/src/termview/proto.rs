...
//! single place that knows their shape, so the viewer's event loop deals in
//! typed values and a change upstream lands in one file.
//!
...
//!   quietly. [`KeyEvent::to_json`] omits, and a test holds it to that.
