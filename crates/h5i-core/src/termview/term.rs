...
//! shows no cursor. That is a worse outcome than whatever the original error
//! was, and "remember to call `restore()`" is not a mechanism. [`Guard`] holds
//! the saved `termios` and undoes every mode it set, on every path out
//! including a panic.
//!
//! Mouse reporting is deliberately *not* enabled for the whole session. It is
//! turned on when the human is driving the page and off again when they are
//! only watching, because a terminal in mouse-reporting mode has taken the
//! mouse away from the terminal emulator: no text selection, no scrollback with
//! the wheel, no copy. Watching is the common case, and it should leave the
//! terminal feeling like a terminal.
