...
pub async fn bind_joiner() -> Result<Endpoint, H5iError> {
    ...
    // connection the joiner established: quinn's defaults allow 100 bidi and 100
    // uni, each with about 1.25 MB of receive window, and data on a stream nobody
    // accepts stays buffered. Roughly 250 MB of the joiner's memory, from a
    // sharer doing nothing but opening streams. iroh's own documentation says
    // protocols that forbid remotely-initiated streams should set both to zero.
    let transport = iroh::endpoint::QuicTransportConfig::builder()
...
/// unconditionally, `h5i box share` hung before printing anything on an offline
/// or relay-blocked network, including the LAN-only case, where the direct
/// addresses it was waiting to supplement were already there, and including
/// `--direct-only`, which had come to depend on reaching the very relay it
/// promises not to put application traffic on.
...
/// correct for all of them and panics for the first one that is not. This is a
/// `pub` helper called from the connection-accept path, where a panic is the
...
mod tests {
    ...
    async fn a_ticket_presented_once_keeps_an_idle_joiner_alive() {
        ...
        // because an endpoint anyone can dial must not be holdable for free. But
        // the normal way this feature is used is: send someone a ticket, they run
        // `h5i join`, and *then* they open the browser. That person was being cut
        // off thirty seconds in, for doing nothing wrong.
        //
        // Time is paused, so this asserts about the grace itself rather than about
        // how long a test is willing to sit still.
        tokio::time::pause();
        let port = fake_dev_server();
        let dir = tempfile::tempdir().expect("tempdir");
        let (bridge, secret) = test_bridge(dir.path(), port);
    ...
    async fn a_flood_at_the_front_door_is_recorded_as_a_flood() {
        ...
        // share was hammered" from "somebody guessed at tickets", and every test
        // that exercised the sentence built the `Summary` by hand, so the refusal,
        // the increment, and the fact that a refused connection costs no task were
        // all unverified.
        //
        // Two different numbers on purpose: an endpoint anyone can dial is refused
        // *before* a credential is asked for, so this must not land in the denial
        // list.
        let port = fake_dev_server();
        let dir = tempfile::tempdir().expect("tempdir");
        let (bridge, secret) = test_bridge(dir.path(), port);
