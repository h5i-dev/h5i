...
async fn bind_loopback(port: u16) -> Result<(tokio::net::TcpListener, bool), H5iError> {
    ...
    // Then the addresses somebody would have *configured*, which is the only way a macOS
    // machine has one of these at all: `sudo ifconfig lo0 alias 127.0.0.2`.
    for d in 2..=9u8 {
...
mod tests {
    ...
    /// The fallback is a decision, and it belongs to the person whose machine it is.
    #[tokio::test]
