...
struct Driver {
    page: h5i_browser::engine::Page,
    last: h5i_browser::snapshot::Snapshot,
}
...
impl Driver {
    ...
    fn click(&mut self, reference: &str) -> h5i_browser::snapshot::Delta {
        let node = self.node(reference);
        self.page.dispatch_event(node, "click");
        self.settle()
    }

    fn type_into(&mut self, reference: &str, text: &str) -> h5i_browser::snapshot::Delta {
        let node = self.node(reference);
        assert!(self.page.type_into(node, text), "typing was refused");
        self.settle()
    }

    fn settle(&mut self) -> h5i_browser::snapshot::Delta {
        self.page.refresh();
