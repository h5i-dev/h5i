...
//!
//! `cargo bench -p h5i-browser --bench read`
...
use h5i_browser::read_ir::ReadTree;
use h5i_browser::receipt::MemorySink;
use h5i_browser::snapshot::Snapshot;
...
impl Bench {
    ...
    fn measure(&self, name: &str, html: &str, mutated: Option<&str>) -> Row {
        ...
        let (_, selector_allocs) = counted(|| {
            let mut cache = h5i_browser::selector::Cache::new();
            walker
                .refs
                .iter()
                .map(|entry| {
                    h5i_browser::selector::for_node_cached(&doc, entry.node_id, &mut cache)
                })
                .collect::<Vec<_>>()
        });
        let selectors = median(|| {
            ...
            for entry in &walker.refs {
                std::hint::black_box(h5i_browser::selector::for_node_cached(
                    &doc,
                    entry.node_id,
                    &mut cache,
                ));
