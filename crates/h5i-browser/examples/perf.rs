...
fn factory(script: bool) -> (PageFactory, Arc<LocalBroker>) {
    let broker = LocalBroker::new(Policy::new(), Arc::new(MemorySink::new()), None)
        .expect("broker");
    let fonts = h5i_browser::fonts::load(
        &[],
        &h5i_browser::fonts::default_font_dirs(),
        Some(2),
    );
...
fn main() {
    println!("h5i-browser — what it costs\n");
    ...
    // built anywhere above would already have paid the compile.
    {
        let phases = std::thread::spawn(|| {
            ...
            let first = h5i_browser::script::Script::new(page.dom(), broker.clone(), &url)
                .expect("realm")
                .cost();
            let later = h5i_browser::script::Script::new(page.dom(), broker.clone(), &url)
                .expect("realm")
                .cost();
            (first, later)
        })
    ...
    // is run from scratch for every realm, even though it is compiled only once.
    {
        ...
        let start = time(5, || {
            let _ = h5i_browser::script::Script::new(page.dom(), broker.clone(), &url)
                .expect("realm");
        });
    ...
    // ── queries and collections ─────────────────────────────────────────────
    {
        ...
        let mut script =
            h5i_browser::script::Script::new(page.dom(), broker, &url).expect("realm");
    ...
    let mut script = h5i_browser::script::Script::new(page.dom(), broker, &url)
        .expect("realm");
