...
Update (2026-07-21): **sealed test sets** shipped, closing the verifier's remaining trust gap — the neutral verifier ran whatever test files were in the candidate's *own* tree, so a coder handed tests by a test-designer could weaken them and still pass (the AgentCoder shape; previously guarded only at the prompt level). `team::verify` now takes an optional `tests_from` (submission id or team agent id, self-sealing fails closed): the sealing submission's base..commit diff is **overlaid over the candidate in the verify worktree after its patch applies**, so sealed content always wins — a candidate's edit to a sealed path is discarded and recorded on the `TeamVerification` as `tests_overridden` (tamper evidence), alongside `tests_source` (`submission`|`sealed`), `tests_from`, `tests_tree_oid`, and the pinned `tests_paths`; the evidence capture's `git_tree` records the tree the command *actually* ran against. A faithful candidate needn't carry the tests at all — the overlay supplies them. `default_verdict` mirrors its divergent-command guard for content: once any eligible candidate is sealed, all must be sealed against the same tests tree or the run records `no_verdict`. The sealed set is exactly the sealing submission's diff vs the run base (a designer wanting an existing test or harness config pinned commits it), and the default single-agent flow is untouched — tests remain part of the deliverable unless a run opts in. Surfaces: `h5i team verify --tests-from`, `Conductor::verify_with_tests`, `.verify_tests_from(...)` on `ensemble`/`integrate`/`arena`, and a `tests_from` param on RPC `conductor.verify` for the Python SDK.
