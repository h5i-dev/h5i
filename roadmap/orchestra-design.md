...
Status: design accepted; M1 through M4 are implemented in `src/orchestra/` (2026-07-10). Shipped: journal + step keys + zero-re-execution resume on the team event log; `Conductor` with `step`/`patched`/`freeze`/`verify`/`judge`/`apply`/`gate` (durable HITL over i5h replies); `agent().hire()` + `work` (with `with_materials` influence-stamped integration), `review`, `revise`, `ask::<T>` (JSON data turns over the new `team agent reply` spool, with bounded re-ask on parse failure); the `VerdictPolicy` trait (built-in rule shared with `team finalize`); `RuntimeLauncher` with `Attach` (default), `LaunchResident` (tmux), and `FnLauncher` (a headless per-turn launcher is rejected, see §5.1); `patterns::{ensemble, integrate, pipeline, arena, map_reduce, debate}`; and `h5i team trace [--dot]` rendering the recorded DAG. The CLI auto-flow is native too: `h5i team run` drives the whole cycle over `patterns::ensemble` + `Conductor::roster()` (dispatch → attempts → freeze → review/revise → verify → verdict → `--apply` or a durable `--gate`), journal-backed and resumable; `scripts/team-run.sh` and `scripts/team-review.sh` are deprecated. Naming deviations from the sketches below (builder entry point instead of `#[orchestra::main]`, `approves(&review)` helper, `materials_granted` events instead of reusing review grants) are documented in the module docs. Design-review hardening (2026-07-10) also shipped: a journal in-flight guard (concurrent same-label steps fail closed) + `Conductor::scope` for parallel step loops; `WorkRequest::expect_independent` runtime validation (used by ensemble/arena first attempts); `Conductor::preflight` (live-session lock probe, isolation floor, clean-worktree — wired into `h5i team run`); `patterns::judge_panel` (LLM judgment over recorded evidence with citation validation + bounded re-ask); an `orchestra::manifest` TOML→pattern bridge (`h5i team run --manifest`: parameters only, unknown patterns refused); and per-step `duration_ms` on the journal, rendered in `team trace`. A live acceptance harness exists (`tests/orchestra_live.rs`, opt-in `H5I_TEST_LIVE_TEAM=1`) plus a deterministic cross-process harness (`tests/orchestra_subprocess.rs`, no LLM: a scripted `sh` subprocess plays the agent, proving the host observes real cross-process box→host submissions — the path the in-process unit tests skip). The live runs earned their keep by surfacing five real issues no unit test caught, all now fixed: (a) the in-box `h5i` resolves from PATH to the installed release, so a live run must `cargo install --path .` first or it validates stale team code; (b) workspace-tier `team agent submit` didn't auto-commit the worktree (auto-commit was gated behind the process+/container in-box path), so an agent that edited but didn't `git commit` lost its work to a no-op refusal — now the host submit branch auto-commits when boxed; (c) Claude Code's folder-trust prompt on a fresh worktree (pre-accepted by a scoped `TrustGuard` in the test); (d) `approves()` only matched the literal first token, so an agent's `Verdict: approve` header read as not-approved and triggered a spurious revise round — now delabeled + leading-token matched; (e) `revise()` waited for a *changed* submission id and deadlocked when an agent found nothing to fix and re-submitted the same candidate — now completes on any new submission event. **With all fixes, the gate is green: `live_team_run_full_cycle` passes in ~79s against two real Claude sonnet sessions (full cycle + journal resume), so the eDSL is operationally proven end-to-end, not just architecturally plausible.** Still open: M5 distribution hardening, journal compaction, token-cost accounting (wall time lands now), and a green live run once the runtime's folder trust is pre-accepted. Companion to `MANUAL.md` ("h5i team") and `docs/environments-design.md`.

Update (2026-07-11): the "scripting mirror" contemplated in §10.2 shipped as a **stdio JSON-RPC bridge + Python SDK**, not a generated binding. `h5i orchestra serve` (`crates/h5i-orchestra/src/rpc.rs`) speaks line-delimited JSON-RPC 2.0 on stdin/stdout — no socket, no daemon (§7 stands), one run per child process, multiplexed requests so `asyncio.gather` maps to concurrent turns. Everything that must live in one process (the Conductor, the journal's per-label sequence counters and in-flight fail-closed checks, turn-wait pollers) stays in the child; the SDK holds only control flow. Client-side closures get begin/commit pairs (`step_begin`/`step_commit`, `judge_begin`/`judge_commit`) so `Conductor::step` and custom `VerdictPolicy`s work across the boundary with the same journal keys; `launcher: "client"` forwards `on_turn` to the SDK (how SDK tests script deterministic agents, mirroring `FnLauncher`); an `initialize` handshake (protocol version + capabilities) decouples SDK and binary release cadence; `ConductorBuilder::score_digest_override` lets the bridge record the *Python file's* digest as run provenance. The SDK lives in the sibling `h5i-python` repo (`h5i.orchestra`, stdlib-only, asyncio-native) with the patterns layer ported to public Python and a cross-process acceptance suite mirroring `tests/orchestra_subprocess.rs`.

Update (2026-07-21): **sealed overlays** shipped, closing the verifier's remaining trust gap — the neutral verifier ran whatever test files were in the candidate's *own* tree, so a coder handed tests by a test-designer could weaken them and still pass (the AgentCoder shape; previously guarded only at the prompt level). `team::verify` now takes an optional `sealed_from` (submission id or team agent id, self-sealing fails closed): the sealing submission's base..commit diff is **overlaid over the candidate in the verify worktree after its patch applies**, so sealed content always wins — a candidate's edit to a sealed path is discarded and recorded on the `TeamVerification` as `sealed_overridden` (tamper evidence), alongside `sealed_from`, `sealed_tree_oid`, and the pinned `sealed_paths`; the evidence capture's `git_tree` records the tree the command *actually* ran against. A faithful candidate needn't carry the sealed files at all — the overlay supplies them. `default_verdict` mirrors its divergent-command guard for content: once any eligible candidate is sealed, all must be sealed against the same tree or the run records `no_verdict`. The sealed content is typically a test set but the mechanism is deliberately content-agnostic (golden files, scoring harnesses, protected interface specs seal identically — hence the neutral naming). The sealed set is exactly the sealing submission's diff vs the run base (a designer wanting an existing test or harness config pinned commits it), and the default single-agent flow is untouched — tests remain part of the deliverable unless a run opts in. Surfaces: `h5i team verify --sealed-from`, `Conductor::verify_sealed`, `.verify_sealed_from(...)` on `ensemble`/`integrate`/`arena`, and a `sealed_from` param on RPC `conductor.verify` for the Python SDK.
...
| `agent.reflect(&artifact)` | `Review` (self) | `reflect_done` | data reply + `reflection_submitted` — self-feedback, never peer review, no influence edge |
...
- **`reflect` vs `review`.** A review is a peer attestation — reviewer ≠ author, enforced at the data model (`grant_review`/`submit_review` fail closed on self-review). `reflect` is the same critique shape aimed at the agent's OWN submission, answered over the data-reply channel and recorded as `reflection_submitted`: never counted as review/quorum evidence, and never routed through `discuss`, so a refinement loop's revisions stay stamped `independent`. This is Self-Refine on one seat: `work → reflect → revise` until the agent's own critique approves.
...
4. The score ingests the spool (`sync_outbound`), the frozen `TeamArtifact` lands as a journal event, and the `work()` future resolves with it. Completion is detected by interval-polling the event log between spool drains, the same pattern as `msg wait` and the team hooks (the repo carries no file-watch dependency; dispatch latency comes from the resident session, not the host poll).
