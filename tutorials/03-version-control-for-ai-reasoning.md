...
`h5i recall context` is a versioned reasoning trail — a structured log of OBSERVE/THINK/ACT steps attached to your repository. It lives in `refs/h5i/context`, follows your code when you `h5i share push`, and snapshots automatically on every `h5i capture commit`.
...
- **`h5i recall context init`** — start a new context workspace with a goal
- **`h5i recall context trace`** — add a single reasoning step (OBSERVE, THINK, ACT, or NOTE)
- **`h5i recall context commit`** — checkpoint the current state as a named milestone
...
```bash
h5i recall context init --goal "refactor auth middleware to use new session store"
```
...
This creates `.h5i-ctx/` in your working directory (git-tracked but h5i-managed) and sets the goal. The goal persists. Every future `h5i recall context status` call will show it, every session handoff will include it.
...
```bash
h5i recall context trace --kind OBSERVE "auth middleware reads token from cookie, not header"
h5i recall context trace --kind THINK   "session store should be injected via Arc<RwLock<>> to avoid clone overhead"
h5i recall context trace --kind ACT     "updated SessionMiddleware::new() in src/auth/middleware.rs to accept store"
h5i recall context trace --kind NOTE    "TODO: verify that existing cookie-based clients aren't broken by this"
```
...
```bash
h5i recall context commit "analyzed auth middleware" \
...
The `--detail` field is the key part. It should answer: "if I came back to this task cold tomorrow, what would I need to know?" That's exactly what `h5i recall resume` will show at the start of your next session.
...
## Starting the next session: h5i recall resume
...
── Run 'h5i recall context relevant <file>' before editing a complex file ─
```

`h5i recall resume` assembles all of this from local data — no API call, no external service. It reads git state, the context workspace, and the last session analysis and composes a structured handoff.
...
Every `h5i capture commit` automatically snapshots the context workspace:

```bash
h5i capture commit -m "implement session store injection" \
...
```bash
h5i recall context restore d4e5f6
```
...
```bash
h5i recall notes analyze   # link the session log to HEAD
h5i recall context commit "implemented session store injection" \
  --detail "Arc<RwLock<>> injection complete. Tests not yet updated. See TODO note in trace."
h5i capture memory         # version Claude's memory files
```
...
- `h5i recall resume` assembles the handoff automatically
...
In Part 4 we'll cover advanced context features: reasoning branches for exploring alternatives without losing your main thread, `h5i recall context knowledge` for distilling key insights across all branches, `h5i recall context relevant` for targeted context injection before editing a complex file, and `h5i rewind` for non-destructive recovery when things go sideways.
