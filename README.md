...
In ML, ensembles beat the best single model — diverse estimators cut variance and won a decade of competitions. The same shift is coming to coding agents: architect, implementer, reviewer, security skeptic. But spawn several agents on one repo with **no coordination layer** and you don't get an ensemble — you get a pileup. Six failure modes Git was never built to handle:
...
| **Context contamination** | agents see a peer's output before an independent attempt — diversity collapses | sealed submissions; no peer access until `freeze` |
...
- **New: Agent teams (`h5i team`).** Run several agents on one task in sealed sandboxes, peer-review, verify neutrally, and merge one auditable winner. [Run an ensemble ↓](#run-an-ensemble-60-seconds)
...
- **New: Prompt Maturity Score.** A deterministic, fully-offline **0–100** quality signal for every AI commit's prompt, rolled into the PR brief.
...
| 2 | **dispatch** | one task → every agent, the same prompt to all |
| 3 | **independent** | each agent works **sealed** — no agent sees a peer's attempt |
...
- **Confined sandbox — `h5i env`.** A disposable worktree plus a policy that limits what the code inside can read, write, and reach. Reading `/etc/shadow`, raw sockets, off-allowlist hosts, and `mount`/`ptrace` are blocked by Landlock + seccomp + namespaces (fail-closed); `h5i env create` picks the strongest tier the host can enforce (`workspace` → `process` → `supervised` → `container`) or refuses. *Proves the agent physically **could not** exfiltrate — not "we logged it."*
...
- **Provenance & audit — `h5i capture commit` · `h5i audit`.** Every commit is **prompt-aware**: prompt, model, agent, tests, and decisions ride along in `refs/h5i/notes`, plus a deterministic, **offline 0–100 Prompt Maturity Score**. Deterministic risk triage flags credential leaks, blind edits, and copy-paste — no model in the loop.
