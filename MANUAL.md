...
| Command | Description |
|---------|-------------|
| `h5i env create <name> [--from REV \| --pr N\|URL [--remote R]] [--profile P] [--isolation TIER] [--audit signal\|all]` | Create an env: code branch + worktree + reasoning branch + pinned policy. Base frozen at creation. With no `--isolation` (or `--isolation auto`) it **auto-picks the strongest tier the host can run**; an explicit tier fails closed if the host can't satisfy it. `--audit all` pins `[audit] capture = "all"` in the resolved policy so wrapped in-env commands are recorded even when they succeed with small output. `--pr` bases the env on a **GitHub pull request**: the PR head (`refs/pull/<n>/head`) is fetched from the remote (default `origin`), pinned as the immutable base, and tracked by a local `pr/<n>` branch that becomes the env's parent — so `propose`/`apply`/`rebase` review the PR, and `apply` prints the exact push-back command (`git push origin pr/<n>:<head-branch>`). Needs only `git`; `gh`, when installed, enriches the hint with the PR's head branch name and fork-ness. A pre-existing `pr/<n>` branch pointing elsewhere is refused, never force-moved. |
...
| `h5i env allow [<host>] [--remove]` | Manage the persistent **user-level egress allowlist** (`~/.config/h5i/egress-allow`): extra hosts merged into every container-tier env whose profile already sets `net.egress` — a deny-all profile is **never** widened from outside the digested policy, and the pinned `policy_digest` is untouched (the extras are runtime state, recorded per-run in the capture manifest's egress tally). Rules: exact `api.example.com`, wildcard `.example.com`/`*.example.com`, optional `:port`. Host-side only: the file lives outside every box-granted path and the command **refuses inside an env box**, so a confined agent can't widen its own network grants. Sessions print the effective allowlist at start (`⦿ egress …`) and denied hosts become `egress-denied` findings — `h5i recall search <host>` answers "what did the box try to reach?", and each finding suggests the one-line fix. No `<host>` lists the current rules. |
| `h5i env list [--json]` | List environments on this clone (the fleet view), each with a **live marker** from the pid registry (`●shell pid N` for a live writer session, `◦N observer(s)`, or `⚠stale` when the durable status says `running` but no live session holds the env — a crash leftover). `--json` emits an array of manifests, each enriched with base `drift` and the `live` session array. |
| `h5i env status <name> [--json]` | Lifecycle state + **live sessions** (pid-verified: kind, pid, start, redacted command; flags a stale `running`) + enforced policy (incl. the egress allowlist) + evidence + base drift. `--json` emits the raw manifest. |
...
| `h5i env log <name> [--json]` | The event log (`created`/`exec`/`service`/`proposed`/`applied`/`aborted`/`gc`/`violation`/`secret`). `--json` emits the event records for tooling. |
| `h5i env diff <name> [--stat] [--json]` | Diff the env's work against its pinned base. `--json` emits a structured diffstat with per-file and total insertion/deletion counts. |
...
| `h5i team verify <team> --agent <id> [--isolation TIER] [--tests-from SRC] -- <cmd>` | **Neutral, sandboxed verifier**: replays the frozen candidate into a throwaway worktree at the run base and runs `<cmd>` under the fail-closed `default` build/test profile. `--isolation` (`workspace`/`process`/`supervised`/`container`) defaults to the strongest tier the host can enforce (falls back to `workspace`); the tier is recorded on the verification. `--tests-from` **seals the test set**: `SRC` is a submission id or team agent id (must differ from the candidate's owner) whose base..commit diff is overlaid over the candidate *after* its patch — the candidate's own edits to those paths are discarded (recorded as `tests_overridden` tamper evidence) so it cannot weaken tests it was handed; the verification records the sealed source, tree digest, and pinned paths. |
...
level). `--tests-from <submission-or-agent>` closes it: the named submission's
base..commit diff becomes the **sealed test set**, overlaid over every
candidate in the verify worktree after its patch applies. Sealed content
always wins — a candidate edit to a sealed path is discarded and recorded as
`tests_overridden` (tamper evidence, surfaced rather than silently dropped),
and a faithful candidate needn't carry the tests at all (the overlay supplies
them). Self-sealing fails closed, and `finalize` extends the apples-to-apples
rule to content: once any candidate is verified sealed, every candidate must
be verified against the *same* sealed set (same tests tree digest) or the run
records `no_verdict`. Scope note: the sealed set is exactly what the sealing
submission changed vs the run base — a designer that wants an *existing* test
or harness config file (`conftest.py`, `Cargo.toml`, …) pinned must include it
in its own commit. In the eDSL this is `c.verify_with_tests(&candidate,
&tests.id, cmd, tier)`, or `.verify_tests_from(...)` on the
`ensemble`/`integrate`/`arena` builders.
