...
**This repo also registers `h5i hook observe-bash`** (PostToolUse, matcher `Bash`): every Bash tool call's command + output is automatically stored as a secret-redacted capture when it is large (≥2 KB) or has stderr — *without* changing what you see. So provenance/recall coverage is automatic; reach for `h5i capture run` when you additionally want the **token reduction** (summary instead of raw output in your context).
