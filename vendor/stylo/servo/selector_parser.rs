...
impl<'a, 'i> ::selectors::Parser<'i> for SelectorParser<'a> {
    ...
    fn parse_has(&self) -> bool {
        // h5i: upstream hard-codes `false` here — not a pref, a constant — so
        // `:has()` was unparseable in stylesheets and selectors alike, and
        // 1,527 WPT subtests plus every real page using it (the corpus counts
        // them) failed on the parse. The matching machinery below this parser
        // is the same code Gecko ships, so parsing is the only gate.
        // See vendor/stylo/README-h5i.md for why this copy exists at all.
        true
    }
